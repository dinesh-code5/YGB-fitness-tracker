...
const ExerciseDetailModal = ({ exercise: initialEx, pastWorkouts, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-4">
      <div className="bg-[var(--surface-card)] border border-[var(--surface-border)] rounded-2xl w-full max-w-lg max-h-[88vh] overflow-y-auto">
        <div className="sticky top-0 bg-[var(--surface-card)] border-b border-[var(--surface-border)] p-4 flex items-center justify-between">
          <div>
            <h3 className="font-semibold text-[var(--text-primary)]">{exercise.name}</h3>
            <p className="text-lg text-muted">{exercise.muscleGroup || exercise.muscles?.join(', ')}</p>
          </div>
          <button onClick={onClose} className="text-muted hover:text-[var(--text-primary)] text-2xl leading-none">×</button>
        </div>
        ...
        <div className="p-5 space-y-5">
          {loading ? (
            ...
            <>
              ...
              {exercise.description && (
                <p className="text-lg text-muted leading-relaxed">{exercise.description}</p>
              )}

              {exercise.youtubeId && (
                <div>
                  <p className="text-lg font-semibold text-[var(--text-primary)] mb-2">📺 How to do it</p>
              ...
              {exercise.cues?.length > 0 && (
                <div>
                  <p className="text-lg font-semibold text-[var(--text-primary)] mb-2">✅ Key Cues</p>
                  <ul className="space-y-1.5">
                    {exercise.cues.map((c, i) => (
                      <li key={i} className="flex items-start gap-2 text-lg text-muted">
              ...
              {exercise.mistakes?.length > 0 && (
                <div>
                  <p className="text-lg font-semibold text-[var(--text-primary)] mb-2">⚠️ Common Mistakes</p>
                  <ul className="space-y-1.5">
          ...
          {history.length > 0 && (
            <div>
              <p className="text-lg font-semibold text-[var(--text-primary)] mb-3">📊 History in past workouts</p>
              <div className="space-y-3">
                {history.slice(0, 5).map((h, i) => (
                  <div key={i} className="card-elevated rounded-xl p-3">
                    <div className="flex items-center justify-between mb-2">
                      <p className="text-lg font-medium text-[var(--text-primary)]">{h.workoutName}</p>
                        ...
                        <div key={si} className="flex items-center justify-between text-xs">
                          <span className="text-muted">
                            ...
                            {s.type && s.type !== 'normal' && (
                              <span className={`ml-1 px-1 rounded text-[10px] ${SET_TYPES.find(t => t.v === s.type)?.bg || ''} ${SET_TYPES.find(t => t.v === s.type)?.color || ''}`}>
  ...
  return (
    <>
      <div className="card rounded-2xl overflow-hidden border-[#2A2A3A] hover:border-[#3A3A4A] transition-all">
        {/* Card Header */}
        <div className="flex items-center gap-4 p-5 bg-[#1E1E2A]">
          <button
            onClick={() => setShowDetail(true)}
            className="w-12 h-12 rounded-xl bg-brand/10 flex items-center justify-center flex-shrink-0 hover:bg-brand/25 transition-colors"
          >
            <GiMuscleUp className="text-brand text-xl" />
          </button>

          <button onClick={() => setShowDetail(true)} className="flex-1 min-w-0 text-left">
            <p className="font-bold text-[#F0F0F5] text-base truncate hover:text-brand transition-colors">
              {exercise.name}
            </p>
            <p className="text-xs text-muted font-medium uppercase tracking-wider mt-0.5">
              {exercise.muscleGroup || exercise.muscles?.slice(0, 2).join(', ')}
              {completedCount > 0 && (
                <span className="text-brand ml-2">· {completedCount}/{exercise.sets.length} SETS DONE</span>
              )}
            </p>
          </button>
          ...
          >
            {collapsed ? <FiChevronDown className="text-xl" /> : <FiChevronUp className="text-xl" />}
          </button>
        ...
        {globalActiveWorkout && (
          <div className="card border-brand/30 bg-brand/5 p-4 mb-5 rounded-xl animate-fade-in">
            <div className="flex items-start justify-between gap-3">
              ...
              <button onClick={handleResume} className="btn-primary text-lg py-2 px-4 flex-shrink-0">
                Resume
              </button>
            </div>
          </div>
    ...
    <div className="page-container max-w-3xl">
      ...
      <div className="flex items-start justify-between mb-10 gap-6">
        <div className="flex items-center gap-6">
          <div className="w-16 h-16 rounded-3xl bg-brand/10 border border-brand/20 flex items-center justify-center text-4xl shadow-glow-sm">
            {currentMood.e}
          </div>
          <div>
      ...
      <div className="space-y-3 mb-4">
        {exercises.length === 0 && (
          <div className="card p-8 text-center border-dashed">
            ...
            <p className="text-muted text-lg">Add your first exercise below</p>
